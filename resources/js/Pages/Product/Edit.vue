...
let cover: Ref<string | null> = ref(props.product.cover);
let images: Ref<string[] | null> = ref(props.product.images);

console.log(props.product.images, images);

const setCover = (event: Event) => {
  const target = event.target as HTMLInputElement;
  editProductForm.cover = target.files?.item(0) ?? null;
  cover.value = editProductForm.cover ? URL.createObjectURL(editProductForm.cover) : null;
};
const unsetCover = () => {
  editProductForm.cover = null;
  cover.value = null;
};

const setImages = (event: Event) => {
  const target = event.target as HTMLInputElement;
  editProductForm.images.push(...Array.from(target.files ?? []));
  ...
  console.log(editProductForm.images, images);
};
const unsetImages = (index: number) => {
  editProductForm.images.splice(index, 1);
                ...
                <CardContent>
                  <div class="grid gap-6">
                    <div class="grid gap-3">
                      ...
                      <Input id="name" type="text" class="w-full" v-model="editProductForm.name" />
                      <InputError :message="editProductForm.errors.name" />
                    </div>
                    <div class="grid gap-3">
                      <Label for="price">Price</Label>
                      <Input type="number" id="price" class="w-full" v-model="editProductForm.price" />
                      <InputError :message="editProductForm.errors.price" />
                    </div>
                    <div class="grid gap-3">
                      <Label for="description">Description</Label>
                      <Textarea id="description" class="min-h-32" v-model="editProductForm.description" />
                      <InputError class="mt-2" :message="editProductForm.errors.description" />
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div class="grid gap-6 sm:grid-cols-3">
                    <div class="grid gap-3">
                      ...
                      <Select v-model="editProductForm.category">
                      ...
                      <InputError class="mt-2" :message="editProductForm.errors.category" />
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div class="grid gap-6">
                    <div class="grid gap-3">
                      ...
                      <InputError class="mt-2" :message="editProductForm.errors.status" />
                    </div>
                  </div>
                </CardContent>
              </Card>
              <Card class="overflow-hidden">
                ...
                <CardContent>
                  ...
                  <div class="grid gap-2">
                    ...
                    <p class="text-sm text-muted-foreground">Other images you can add to your product.</p>
                    <div class="grid grid-cols-3 gap-2">
                      ...
                      <label class="flex aspect-square w-full items-center justify-center rounded-md border border-dashed mb-4">
                        <Upload class="h-4 w-4 text-muted-foreground" />
                        <span class="sr-only">Upload</span>
                        <input type="file" accept=".png, .jpg, .jpeg" multiple class="hidden" @input="setImages" />
                      </label>
                    </div>
                  </div>
                </CardContent>
